...
function tap.packet(pinfo,tvb,tapinfo)
	...
	if get_nbns_opcode() == 0 and get_nbns_response() == 1 then
		-- Name Query Response
		-- FIXME Is this a positive or a negative response?
		--packetinfo:set_multi_attribute("netbios.name", get_nbns_src_name())
	elseif get_nbns_opcode() == 5 and get_nbns_response() == 0 then
		-- Registration Query
		--packetinfo:set_multi_attribute("netbios.name", get_nbns_src_name())
	end
end
